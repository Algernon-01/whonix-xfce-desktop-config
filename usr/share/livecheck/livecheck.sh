...
#live mode is enabled
	...
	echo "<txt>Live</txt>"
	echo "<tool>Live mode is enabled. All changes to the disk will be gone after a reboot. See: https://whonix.org/wiki/Whonix_Live</tool>"
	else
	#disk is not set to RO
	echo "<img>/usr/share/icons/Adwaita/16x16/status/dialog-warning.png</img>"
	echo "<txt>Live</txt>"
	echo "<tool>Live mode is enabled but it is still possible to write to the disk. Please power off the machine and set the disk to read-only. See: https://whonix.org/wiki/Whonix_Live</tool>"	
fi
else
#Live mode is disabled
echo "<img>/usr/share/icons/gnome-colors-common/22x22/status/gtk-info.png</img>"
echo "<tool>You are using persistent mode. All changes to the disk will be preserved after a reboot. For using live mode, see: https://whonix.org/wiki/Whonix_Live</tool>"
fi
